...
The Social nodes decouple your workflow from any single platform. **Social Receive** sits after a platform trigger (WhatsApp Receive, Telegram Receive, Discord Receive, Chat Trigger) and normalizes whatever arrives into one schema; **Social Send** takes one payload shape and routes it to the platform selected by its `channel` parameter. Build the middle of your workflow once against the unified schema, and swap the endpoints per platform.

### Social Send

Platform-agnostic outbound messaging. The node advertises eleven channels (`whatsapp`, `whatsapp_business`, `telegram`, `discord`, `slack`, `signal`, `sms`, `webchat`, `email`, `matrix`, `teams`). Three are wired today — `whatsapp` (personal account), `whatsapp_business` (Meta Cloud API), and `discord` — and the rest return an error naming the platforms that are available. Use **Telegram Send** for Telegram in the meantime. Discord accepts `text` messages only through this node; use **Discord Send** for embeds and attachments.
...
<ParamField path="channel" type="select" default="whatsapp">
  Target platform. `whatsapp`, `whatsapp_business`, and `discord` are wired today.
</ParamField>
...
Connect a platform trigger (WhatsApp Receive, Telegram Receive, Discord Receive, Chat Trigger) to Social Receive; it detects the source platform from the upstream node and normalizes automatically.
